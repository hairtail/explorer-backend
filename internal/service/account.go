...
func (e *Service) GetAccountActiveSmeshers(ctx context.Context, accountID string, page, perPage int64, layer uint32) ([]*model.Smesher, int64, error) {
	addr, err := address.StringToAddress(accountID)
	if err != nil {
		return nil, 0, ErrNotFound
	}
	opts := e.getFindOptions("coinbase", page, perPage)
	opts.SetProjection(bson.D{})
	return e.getActiveSmeshers(ctx, &bson.D{{Key: "coinbase", Value: addr.String()}}, layer, opts)
}
